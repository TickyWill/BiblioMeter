...
def _set_otps_dept_affil(org_tup, in_df, otps_homonyms_cols_dic):
    ...
    Args:
        ...
        with a row per Institute's author and their attributes columns.
...
def _add_authors_name_list(org_tup, in_df, otps_homonyms_cols_dic):
    ...
    (pub_id_col, author_id_col, mat_col, lastname_col, firstname_col,
     ...
     fullname_col, institute_auths_col) = [otps_homonyms_cols_dic[key] for key in use_keys_list]
    ...
    for _, pub_id_df in in_df.groupby(pub_id_col):
        ...
        pub_id_df[institute_auths_col] = authors_full_str
...
def _set_add_otps_cols_dic(institute, org_tup):
    ...
    It also builds the full list of final column names got from 
    the same function.
    ...
    Returns:
        (tup): The built dict and the built full list of final column names.
    """
    # Setting useful column names
    otp_col_dic = set_otp_col_names(institute, org_tup)
...
def _set_input_data(add_otp_params, in_path):
    ...
    return add_otps_cols_tup, init_pub_df, dpt_attributs_dict, dpt_list, nolab_depts
...
def _save_dpt_otp_file(dpt, save_otp_cols_tup, dpt_df, dpt_otp_list, xl_dpt_path):
    ...
    of the publications for the Institute's department labeled 'dpt'.
    ...
    Arg:
        dpt (str): Institute's department.
        ...
        dpt_otp_list (list): List of Institute's departments (str).
...
def _save_dpt_lab_otp_file(institute, dpt, save_otp_cols_tup, dpt_df,
    ...
    with values set to the validation list. 
    The dataframe columns are renamed using 'add_otps_cols_tup'. 
    Then the dataframe is formatted as a multisheet openpyxl workbook through
    ...
    Arg:
        ...
        dpt (str): The department's label.
        ...
        dpt_df (dataframe): The publications-list data of a department of the Institute.
...
def _set_otp_lab(otp_sub_params, cols_list, dpt_labs_list, lab_df, lab):
    ...
    """
    # Set parameters value from args
    institute, nolab_depts = otp_sub_params
...
def _build_otp_dept_df(otp_sub_params, build_otp_cols_list, dpt_attributs_dict,
    ...
    dpt_filter = False
    for dpt_value in dpt_attributs_dict[dpt][bm_ig.DPT_LABEL_KEY]:
        dpt_filter = dpt_filter | (full_df[dpt_col]==dpt_value)
    dpt_df = full_df[dpt_filter].copy()
    ...
    if len(dpt_df):
        for lab, lab_df in dpt_df.groupby(lab_col):
            otp_lab = _set_otp_lab(otp_sub_params, sub_cols_list,
...
def _set_full_pub_df(init_pub_df, add_otps_cols_dic, dpt_list):
    """Adds information about the Institute's departments to the list of publications
    with one row per author.

    To each department's column, 1 value is assigned if one co-author, at least, is member
    of this department, otherwise the value assigned is 0. The author's detailed information
    is given for the first author to support the user in the attribution of the OTP.
...
def _add_lab_otp(add_otp_params, in_path, out_path, out_file_base, lab_otps_dict):
    ...
    for dpt in sorted(dpt_list):
        ...
        otp_dpt_df = _build_otp_dept_df(otp_sub_params, build_otp_cols_list,
...
def add_otp(otp_params, in_path, out_path, out_file_base):
    """Creates the files for setting OTP attribute of publications by the user 
    for the Institute's departments either among OTPs list at department level 
    or lab level.
