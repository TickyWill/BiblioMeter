...
def _formatting_wb_sheet(institute, org_tup, if_sheet_name, df, wb, first, if_database):
    """Formats impact-factors (IFs) sheet in the 'wb' Openpyxl workbook through the `mise_en_page` function 
    imported from the `bmfuncts.useful_functs` module as first sheet of the workbook if first is True.
...
def _get_if(if_updated_file_path, useful_col_list,
    ...
    """Gets the impact-factors (IFs) dataframe from the file updated with IFs values which full path 
    is given by 'if_updated_file_path'.
...
def _update_year_if_database(institute, org_tup, bibliometer_path,
    ...
    """Updates the dataframe of impact_factors (IFs) database of a year with the IFs dataframes 
    extracted, through the `_get_if` internal function, from the files which full paths
    are given by 'year_missing_if_path' and 'year_missing_issn_path'.
    Then, initializes the dataframe of IFs database of the most recent year.
    ...
    Returns:
        (tup): Tuple = (fully updated IFs database of the given year (dataframe), 
...
def _build_previous_years_if_df(institute, org_tup, bibliometer_path,
    ...
    First, initializes the dataframe to add for building the IFs database of the most-recent year.
    Then, for each IFs-year, the steps are as follows:
        - Gets the initial database of the IFs-year from the all-years database 
        and capitalizes the journal-names main words through the `_capwords_journal_col` 
        internal function.
        - Builds the fully updated dataframes of IFs database for the IFs-year and  
        ...
        - Appends the partial dataframe of most-recent-year IFs to the dataframe to add 
        for building the IFs database of the most-recent year.
        - Formats IFs sheet in the 'wb' Openpyxl workbookthrough the `_formatting_wb_sheet` 
...
def _build_recent_year_if_df(institute, org_tup, bibliometer_path,
    ...
    First, initializes the dataframe of the IFs database of the most-recent year 
    ...
    For each corpus year in the 'off_if_db_years_list' years list, the steps are as follows:
        - Builds the partial dataframe of most-recent-year IFs limited to the journals data 
    ...
    Then, appends the resulting dataframe of the loop to the initial dataframe of the IFs database 
    of the most-recent year .
    Finally, formats IFs-most-recent-year sheet in the 'wb' Openpyxl workbook through  
...
def update_inst_if_database(institute, org_tup, bibliometer_path,
    ...
    To do that:
        - First, Gets the initial all-years database which full path is given by 
        'inst_all_if_path'.
        - Then, sets useful parameters for using `_build_previous_years_if_df` and 
        `_build_recent_year_if_df` internal functions, including 'wb' workbook for 
        saving updated IFs database as multisheet workbook.
        - Then, updates 'wb' workbook through the `_build_previous_years_if_df` 
        internal function to build fully updated IFs database for years before 
        the most recent year available for IFs.
        - Then, updates 'wb' workbook through the `_build_recent_year_if_df` 
        internal function to build fully updated IFs database for years beginning
        from the most recent year available for IFs.
        - Finally, saves the 'wb' workbook using the 'inst_all_if_path' full path.
