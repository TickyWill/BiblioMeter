...
def _check_names_spelling(bibliometer_path, init_df, cols_tup):
    ...
    new_df = init_df.copy()
    for pub_row_num, _ in init_df.iterrows():
        lastname_init = init_df.loc[pub_row_num, col1]
        initials_init = init_df.loc[pub_row_num, col2]
...
def recursive_year_search(out_path, empl_df, institute, org_tup, bibliometer_path,
    ...
    # Keeping history of publication author names and corresponding employee name
    _save_author_employee_df(submit_path, out_path)
    if progress_callback:
        progress_callback(new_progress_bar_state + step * 5)
