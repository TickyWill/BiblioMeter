...
def _get_input_data(input_data_params, bp_cols_list):
    ...
    Args:
        input_data_params (list):  The list composed of the Institute name (str), the org_tup (tup) \
    ...
    if datatype=="Scopus-HAL & WoS":
        ...
        check_params = [corpus_year, institute, org_tup, wf_path]
        return_tup = _check_added_dois_affil(check_params, dfs_list, bp_cols_list)
    ...
    return articles_df, authors_df, authaddr_df
...
def _reorder_cols(institute_merged_df, reorder_cols_list):
    ...
    Args:
        institute_merged_df (dataframe): Data of publication IDs list \
...
def build_institute_pubs_authors(params_list):
    """Builds the publications list dataframe with one row per Institute's author 
    ...
    the `_recasting_authors_df` internal function.
    ...
    is built using the 'filt_auth_affil' filter got through \
    the `_build_filt_auth_affil` internal function.
    ...
    print_step_text("  - Building publications list with authors affiliated "
    ...
    drop_articles_path, drop_authaddr_path, ortho_path, complements_path = paths_list
    ...
    authors_df = _check_names_spelling(authors_df, ortho_path, bm_auth_names_list,
                                       bm_ortho_cols_list, print_params)
    ...
    institute_merged_df = authaddr_auth_df[filt_auth_affil].merge(new_articles_df, how='left',
    ...
    institute_merged_df = _reorder_cols(institute_merged_df, reorder_cols_list)
