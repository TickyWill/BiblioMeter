...
__all__ = ['correct_dedup',
           'initialize_false_addresses_file',
          ]
...
def _set_dedup_cols_dic():
    ...
    Returns:
        (dict): The built dict.
    """
    dedup_cols_dic = {'bp_pub_id_col'      : bp.COL_NAMES['pub_id'],
...
def _set_correct_dedup_paths(final_results_path, corpus_year, item_filename_dict="", test_txt=""):
    ...
    false_addresses_file_alias = bm_pg.ARCHI_RESULTS["False_address_file"]
    ...
    paths_list = []
    false_addresses_path = dedup_parsing_path / Path(false_addresses_file_alias)
    paths_list.append(false_addresses_path)
...
def initialize_false_addresses_file(params_list, file_clean=False):
    # internal functions
    def _save_empty_file():
        ...
        wb.save(correct_addresses_path)
    ...
    correct_addresses_path = correct_paths_list[0]
    ...
    (pub_id_col, address_id_col, address_col,
     country_col, correct_address_col) = [dedup_cols_dic[key] for key in cols_keys]

    correct_addresses_cols = [pub_id_col, address_id_col, address_col,
                              correct_address_col, country_col]

    # Creating or cleaning the false-addresses file
    if correct_addresses_path.is_file() and not file_clean:
        message = ("\n    File for correcting false addresses already exist")
    else:
        _save_empty_file()
        if file_clean:
            message = ("\n    File for correction of false addresses by the user cleaned")
        else:
            message = ("\n    Empty file for correction of false addresses created")
    print(message)
    return correct_addresses_path


def _add_auth_ids_to_false_address_data(all_correct_dfs, dedup_cols_dic):
    ...
    addresses_df, countries_df, auth_inst_df, correct_addresses_df = all_correct_dfs

    # Setting useful column names    
    dedup_cols_dic = _set_dedup_cols_dic()
    cols_keys = ['bp_pub_id_col', 'bp_address_id_col', 'bp_address_col',
                 'bp_country_col', 'bp_author_id_col',
                 'author_ids_col', 'correct_address_col']   
    (pub_id_col, address_id_col, address_col, country_col, author_id_col,
     author_ids_col, correct_address_col) = [dedup_cols_dic[key] for key in cols_keys]

    correct_addresses_cols = [pub_id_col, address_id_col, address_col,
                              correct_address_col, country_col, author_ids_col]
    data = []
    for _, corr_row in correct_addresses_df.iterrows():
        pub_id = corr_row[pub_id_col]
        ...
        data.append([pub_id, address_id, false_address, correct_address,
                     country, false_address_auth_ids])
    new_correct_addresses_df = pd.DataFrame(data, columns=correct_addresses_cols)
    return new_correct_addresses_df


def _correct_dedup_countries(countries_correct_dfs, parse_cols_dic, parsing_countries_path):
    ...
    for pub_id, pub_id_df in countries_df.groupby(pub_id_col):
        ...
        if pub_id in correct_pub_ids_list:
            pub_id_correct_address_df = correct_addresses_df[correct_addresses_df[pub_id_col]==pub_id]
    ...
    message = ("\n        False addresses corrected in countries parsing data")
    print(message)
    return new_countries_df


def _correct_dedup_addresses(addresses_correct_dfs, parse_cols_dic, parsing_addresses_path):
    """Corrects the parsing data of countries using the data of unknown countries corrected by the user.
...
def _correct_dedup_authsinst(authsinst_correct_dfs, parse_cols_dic,
    ...
    message = ("\n        False addresses corrected in authors-institutions parsing data")
    print(message)
    return new_auth_inst_df


def correct_dedup(params_list, item_filename_dict, test_txt=""):
    ...
    (correct_addresses_path, parsing_countries_path,
    ...
    if correct_addresses_path.is_file():
        init_correct_addresses_df = pd.read_excel(correct_addresses_path)
        if not init_correct_addresses_df.empty:
            print("\n    Correcting false addresses in deduplication-parsing data...")
            # If data of the user's correction of the false addresses not empty,
            # proceeding with deduplication parsing data correction

            # Getting the dict of deduplication results
            parsing_dict = read_final_dedup(wf_path,
                                            final_results_path,
                                            corpus_year)
            ...
            all_correct_dfs = [addresses_df, countries_df, auth_inst_df, init_correct_addresses_df] 
            correct_addresses_df = _add_auth_ids_to_false_address_data(all_correct_dfs, dedup_cols_dic)

            # Correcting the countries parsing data using the user's correction of the addresses
            countries_correct_dfs = [countries_df, correct_addresses_df]
            _ = _correct_dedup_countries(countries_correct_dfs, dedup_cols_dic, parsing_countries_path)

            # Correcting the addresses parsing data using the user's correction of the addresses
            addresses_correct_dfs = [addresses_df, correct_addresses_df]
            _ = _correct_dedup_addresses(addresses_correct_dfs, dedup_cols_dic, parsing_addresses_path)
            ...
            _ = _correct_dedup_authsinst(authsinst_correct_dfs, dedup_cols_dic, parsing_authsinst_path,
                                           norm_dicts)
            correct_status = True
            correct_addresses_path = initialize_false_addresses_file(params_list, file_clean=True)
    else:
        correct_status = False 
        correct_addresses_path = initialize_false_addresses_file(params_list)
    return correct_addresses_path, correct_status
