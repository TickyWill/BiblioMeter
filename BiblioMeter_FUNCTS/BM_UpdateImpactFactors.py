...
def journal_capwords(text):
    ...
    text_split_list = []
    for x in text.split():
        if x.lower() in BM_LOW_WORDS_LIST: 
            x = x.lower()
        else:
            x = x.capitalize()
...
def _update_year_if_database(bibliometer_path, corpus_year, year_if_db_df, 
    ...
    if_appended_year_if_db_df = year_if_db_df.append(missing_if_corpus_year_if_df)
    ...
    fully_appended_year_if_db_df = if_updated_year_if_db_df.append(missing_issn_corpus_year_if_df) 
...
def update_inst_if_database(bibliometer_path, corpi_years_list):
    ...
    for if_db_year in if_db_years_list[:-1]:
        ...
        fully_updated_year_if_db_df, corpus_year_most_recent_year_if_df_to_add = _update_year_if_database(bibliometer_path, 
                                                                                                   corpus_year, 
                                                                                                   year_if_db_df,
                                                                                                   pub_list_path_alias, 
                                                                                                   ...
                                                                                                   most_recent_year)
        wb = _formatting_wb_sheet(if_db_year, fully_updated_year_if_db_df, wb, first, if_database)
        first = False   
        most_recent_year_if_df_to_add = most_recent_year_if_df_to_add.append(corpus_year_most_recent_year_if_df_to_add)                        
    ...
    most_recent_year_if_db_df.fillna(UNKNOWN, inplace = True)
    most_recent_year_if_db_df[journal_col_alias] = most_recent_year_if_db_df.apply(_capwords_journal_col, axis=1)
    ...
    most_recent_year_if_db_df = most_recent_year_if_db_df.append(most_recent_year_if_df_to_add)
    most_recent_year_if_db_df = most_recent_year_if_db_df.drop_duplicates(subset = journal_col_alias, keep = 'last')
    
    wb = _formatting_wb_sheet(most_recent_year, most_recent_year_if_db_df, wb, first, if_database)
