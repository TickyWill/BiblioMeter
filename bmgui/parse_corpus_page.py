...
def _create_table(self, bibliometer_path, pos_x_init):
    ...
    and the tk window general properties as 'AppMain' class variables.
    ...
    ref_font_size = 11
    local_font_size = font_size(ref_font_size, AppMain.width_sf_min)
    ...
    pos_x_shift = mm_to_px(25 * AppMain.width_sf_mm,  gg.PPI)
    pos_y_ref   = mm_to_px(30 * AppMain.height_sf_mm, gg.PPI) # 35
...
def _update(self, bibliometer_path, pos_x, pos_y, esp_ligne):
    ...
    eff_font_size = font_size(11, AppMain.width_sf_min)
    eff_dx        = mm_to_px(1 * AppMain.width_sf_mm,  gg.PPI)
    eff_dy        = mm_to_px(1 * AppMain.height_sf_mm, gg.PPI)


    # Setting existing corpuses status
    files_status = existing_corpuses(bibliometer_path)
    AppMain.list_corpus_year    = files_status[0]
    AppMain.list_wos_rawdata    = files_status[1]
    AppMain.list_wos_parsing    = files_status[2]
    AppMain.list_scopus_rawdata = files_status[3]
    AppMain.list_scopus_parsing = files_status[4]
    AppMain.list_dedup          = files_status[5]

    # Setting useful local variables for default selection items in selection lists
    default_year = AppMain.list_corpus_year[-1]

    # ????
    for i in range(len(self.CHECK)):
        self.CHECK[i].efface()

    for i, annee in enumerate(AppMain.list_corpus_year):
        tmp = CheckBoxCorpuses(self,
                               annee,
                               AppMain.list_wos_rawdata[i],
                               AppMain.list_wos_parsing[i],
                               AppMain.list_scopus_rawdata[i],
                               AppMain.list_scopus_parsing[i],
                               AppMain.list_dedup[i])
        tmp.place(x = pos_x,
                  y = i * esp_ligne + pos_y)
...
def _launch_parsing(self, corpus_year, database_type, bibliometer_path, pos_x, pos_y, esp_ligne):
    ...
    if database_type == 'wos':
        list_rawdata = AppMain.list_wos_rawdata
        list_parsing = AppMain.list_wos_parsing
    elif database_type == 'scopus':
        list_rawdata = AppMain.list_scopus_rawdata
        list_parsing = AppMain.list_scopus_parsing
    else:
        ...
        return
    rawdata_status = list_rawdata[AppMain.list_corpus_year.index(corpus_year)]
    parsing_status = list_parsing[AppMain.list_corpus_year.index(corpus_year)]
...
def _launch_synthese(self, corpus_year, institute, org_tup, bibliometer_path,
    ...
    wos_parsing_status    = AppMain.list_wos_parsing[AppMain.list_corpus_year.index(corpus_year)]
    scopus_parsing_status = AppMain.list_scopus_parsing[AppMain.list_corpus_year.index(corpus_year)]
    dedup_parsing_status  = AppMain.list_dedup[AppMain.list_corpus_year.index(corpus_year)]
...
def create_parsing_concat(self, master, page_name, institute, bibliometer_path, datatype):
    ...
    # Setting useful local variables for positions modification (globals to create ??)
    # numbers are reference values in mm for reference screen
    position_selon_x_check   = mm_to_px(70  * AppMain.width_sf_mm,  gg.PPI)
    position_selon_y_check   = mm_to_px(40  * AppMain.height_sf_mm, gg.PPI)  #40
    espace_entre_ligne_check = mm_to_px(10  * AppMain.height_sf_mm, gg.PPI)
    labels_x_pos             = mm_to_px(10  * AppMain.width_sf_mm,  gg.PPI)
    labels_y_space           = mm_to_px(10  * AppMain.height_sf_mm, gg.PPI)
    status_label_y_pos       = mm_to_px(25  * AppMain.height_sf_mm, gg.PPI)  #25
    parsing_label_y_pos      = mm_to_px(107 * AppMain.height_sf_mm, gg.PPI)
    synthese_label_y_pos     = mm_to_px(135 * AppMain.height_sf_mm, gg.PPI)
    status_button_x_pos      = mm_to_px(148 * AppMain.width_sf_mm, gg.PPI)  #148
    status_button_y_pos      = mm_to_px(98  * AppMain.height_sf_mm, gg.PPI)  #98
    dx_year_select           = mm_to_px(1   * AppMain.width_sf_mm,  gg.PPI)
    dy_year_select           = mm_to_px(1   * AppMain.height_sf_mm, gg.PPI)
    dx_bdd_select            = mm_to_px(12  * AppMain.width_sf_mm,  gg.PPI)  #12
    dy_bdd_select            = mm_to_px(1   * AppMain.height_sf_mm, gg.PPI)
    dx_launch                = mm_to_px(15  * AppMain.width_sf_mm,  gg.PPI)  #15
    dy_launch                = mm_to_px(0.2 * AppMain.height_sf_mm, gg.PPI)
    eff_labels_font_size     = font_size(14, AppMain.width_sf_min)
    eff_select_font_size     = font_size(12, AppMain.width_sf_min)
    ...
    self.var_year_pc_2 = tk.StringVar(self)
    self.var_year_pc_2.set(AppMain.list_corpus_year[-1])
    self.OM_year_pc_2 = tk.OptionMenu(self,
                                      self.var_year_pc_2,
                                      *AppMain.list_corpus_year)
    font_year_pc_2 = tkFont.Font(family = gg.FONT_NAME,
